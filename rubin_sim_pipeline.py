import numpy as np
# this is janky but there's a version mismatch
import os
import pandas as pd
...
def get_rubin_cadence(
    ...
    print('bundle_list[0].metric_values[0]:', bundle_list[0].metric_values[0])
    lsst_cadence = pd.DataFrame(bundle_list[0].metric_values[0])
    print('lsst_cadence.head():', lsst_cadence.head())
    lsst_cadence["observationStartMJD"] = lsst_cadence["observationStartMJD"] - np.min(
        lsst_cadence["observationStartMJD"]
    )
    lsst_cadence = lsst_cadence.sort_values("observationStartMJD").reset_index(drop=True)
    return lsst_cadence
